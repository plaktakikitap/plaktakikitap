        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6 sm:gap-8">
          {books.map((book, i) => (
            <motion.article
              ...
              className="group"
            >
              <div className="relative overflow-hidden rounded-xl border border-amber-400/20 bg-ink/5 backdrop-blur-sm transition-all duration-200 hover:border-amber-400/40 hover:shadow-[0_0_30px_-5px_rgba(212,182,90,0.2)]">
                <button
                  type="button"
                ...
                >
                  <div className="relative aspect-[2/3] bg-gradient-to-b from-[#f5f0e8] to-[#ebe4d9]">
                    ...
                    />
                    {!book.is_released && (
                      <div className="absolute inset-x-0 top-0 flex items-center justify-center bg-black/50 py-2 text-xs font-medium uppercase tracking-wider text-ink/95 backdrop-blur-[2px]">
                  ...
                  <div className={book.is_released && book.amazon_url ? "p-4 pr-11" : "p-4"}>
                    <p className="font-editorial font-bold text-ink line-clamp-2">
                      {book.title}
                    </p>
                    <p className="mt-0.5 text-sm text-ink-muted">{book.original_author}</p>
                ...
                {book.is_released && book.amazon_url ? (
                  <a
                    ...
                    className="absolute bottom-3 right-3 z-10 inline-flex h-7 w-7 items-center justify-center rounded-md text-ink-muted/80 transition hover:bg-amber-500/15 hover:text-amber-800"
                    aria-label={`${book.title} — Amazon’da satın al`}
                  >
    ...
    >
      ...
      >
        <button
          type="button"
          onClick={onClose}
          className="absolute right-3 top-3 z-10 rounded-full p-1.5 text-[var(--muted)] hover:bg-ink/5 hover:text-[var(--foreground)]"
          aria-label="Kapat"
        >
          <X className="h-5 w-5" />
        </button>
        <div className="flex flex-col sm:flex-row">
          ...
          <div className="flex-1 p-5 sm:p-6 flex flex-col">
            <h3 className="font-editorial text-lg font-bold text-cream">
              {book.title}
            </h3>
            <p className="text-sm text-ink-muted mt-0.5">{book.original_author}</p>
            {book.source_lang && book.target_lang && (
              <div className="mt-2">
                <LanguagePill source={book.source_lang} target={book.target_lang} variant="on-dark" />
              </div>
            )}
            ...
            {book.translator_note?.trim() && (
              <div
                className="mt-4 rounded-lg border border-amber-200/60 bg-card dark:border-amber-800/40 dark:bg-[#2c2820] p-4 shadow-inner"
                style={parchmentBg}
              >
            ...
            <div className="mt-auto pt-4">
              {book.is_released && book.amazon_url ? (
                <a
                  ...
                  target="_blank"
                  rel="noopener noreferrer"
                  className="inline-flex items-center gap-2 rounded-lg bg-amber-600/90 px-4 py-2 text-sm font-medium text-ink hover:bg-amber-600 transition"
                >

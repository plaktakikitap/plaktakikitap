...
export function AdminPlanner() {
  ...
  for (let d = 1; d <= daysInMonth; d++) {
    ...
    cells.push(
      <button
        ...
        className={`flex flex-col items-center justify-center rounded py-2 text-sm text-[#1a1612] transition hover:bg-amber-500/20 ${
  ...
  return (
    <div className="space-y-6">
      <header>
        <h1 className="text-3xl font-bold text-[#1a1612]">Planner</h1>
        <p className="mt-1 text-[#1a1612]/70">
      ...
      <section className="rounded-xl border border-[#d4c9bb] bg-[#1a1612]/5 p-4">
        <div className="mb-4 flex items-center justify-between">
          <button
            ...
            className="rounded p-1.5 text-[#1a1612]/70 hover:bg-[#1a1612]/8 hover:text-[#1a1612]"
        ...
        <div className="grid grid-cols-7 gap-1 text-center">
          {["Pzt", "Sal", "Çar", "Per", "Cum", "Cmt", "Paz"].map((d) => (
            <div key={d} className="py-1 text-xs font-medium text-[#6b6158]">
              {d}
            </div>
          ))}
          {cells}
...
const labelClass = "block text-xs font-medium text-[#1a1612]/80 mt-3 first:mt-0";
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 p-4" onClick={onClose}>
      ...
      >
        <div className="sticky top-0 flex items-center justify-between border-b border-[#d4c9bb] bg-[#1a1f2e] px-4 py-3">
          ...
          <div className="flex items-center gap-2">
            ...
            <button
              ...
              className="flex items-center gap-1.5 rounded-lg bg-amber-500 px-3 py-1.5 text-sm text-[#1a1612] hover:bg-amber-600"
            >
        ...
        <div className="p-4 space-y-3">
          <div className="rounded-lg border border-[#d4c9bb] bg-[#1a1612]/5 p-3">
            <div className="flex items-center justify-between">
              <span className="text-sm font-medium text-[#1a1612]/80">Gün notu</span>
              <button
                type="button"
            ...
            {dayEntry && (
              <div className="mt-2 text-sm text-[#1a1612]">
                {dayEntry.title && <p className="font-medium line-clamp-1">{dayEntry.title}</p>}
                {dayEntry.content && <p className="mt-0.5 line-clamp-2 text-[#1a1612]/70">{dayEntry.content}</p>}
                {(dayEntry.tags?.length ?? 0) > 0 && (
                  <p className="mt-1 text-xs text-[#6b6158]">{dayEntry.tags.join(", ")}</p>
                )}
              </div>
            )}
          </div>
          {entries.map((entry) => (
            <div
              key={entry.id}
              className="rounded-lg border border-[#d4c9bb] p-4 text-[#1a1612]"
            >
              <h4 className="font-medium line-clamp-1">{entry.title || "(Başlıksız)"}</h4>
              {entry.summaryQuote && (
                <p className="mt-1 text-xs text-[#6b6158] line-clamp-2">{entry.summaryQuote}</p>
              )}
              <div className="mt-2 flex items-center gap-2">
                {entry.media.some((mm) => mm.attachmentType === "paperclip" || mm.attachmentType === "paste" || mm.attachmentType === "staple" || mm.attachmentStyle) && (
                  <span className="flex items-center gap-1 text-xs text-[#6b6158]">
      ...
      >
        ...
        <form onSubmit={handleSubmit} className="p-4 space-y-3">
          ...
          <div>
            ...
            <button type="button" disabled={photoUploading} className="rounded border border-[#d4c9bb] px-2 py-1 text-sm text-[#1a1612] hover:bg-[#1a1612]/8" onClick={() => fileInputRef.current?.click()}>
          ...
          <div className="flex gap-2 pt-2">
            ...
            <button type="button" onClick={onClose} className="rounded-lg border border-[#d4c9bb] px-4 py-2 text-sm text-[#1a1612] hover:bg-[#1a1612]/8">
              İptal
            </button>
          </div>
        </form>
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 p-4" onClick={onClose}>
      ...
      >
        ...
        <form onSubmit={handleSubmit} className="p-4">
          ...
          {files.length > 0 && (
            <p className="mt-1 text-xs text-[#6b6158]">{files.length} dosya seçildi</p>
          )}

          <label className="mt-3 flex cursor-pointer items-center gap-2 text-sm text-[#1a1612]">
            <input
              type="checkbox"
            ...
            />
            <Paperclip className="h-4 w-4 text-[#6b6158]" />
          ...
          {isEdit && entry.media.length > 0 && (
            <div className="mt-4 space-y-2">
              ...
              <p className="text-[10px] text-[#6b6158] -mt-1">
          ...
          <div className="mt-6 flex gap-2">
            ...
            <button type="button" onClick={onClose} className="rounded-lg border border-[#d4c9bb] px-4 py-2 text-sm text-[#1a1612] hover:bg-[#1a1612]/8">İptal</button>
          </div>
        </form>
      </div>
    </div>

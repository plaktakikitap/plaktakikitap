  ...
  return (
    <div className="grid grid-cols-2 gap-4 sm:grid-cols-4">
        {items.map(({ icon: Icon, label, value, sub }) => (
          ...
          >
            <div className="flex h-12 w-12 shrink-0 items-center justify-center rounded-xl bg-gradient-to-br from-amber-500/20 to-amber-600/10 text-[#b8934a]">
              <Icon className="h-6 w-6" />
            </div>
            <div className="min-w-0 flex-1">
              <p className="text-[10px] font-medium uppercase tracking-wider text-[#6b6158]">
                {label}
              </p>
              ...
              {sub && (
                <p className="text-xs text-[#b8934a]/80">{sub}</p>
              )}
            </div>
          </div>
        ))}

...
export function AddSeriesModal({
  ...
  return (
    <div className="fixed inset-0 z-[90] flex items-center justify-center p-4">
      ...
      />
      <div className="relative w-full max-w-lg overflow-hidden rounded-2xl border border-white/10 bg-[#121212] shadow-2xl">
        <div className="flex items-center justify-between border-b border-white/10 px-5 py-4">
          <h2 className="font-semibold text-white">Dizi Ekle</h2>
          <button
            type="button"
            onClick={onClose}
            disabled={adding}
            className="rounded-lg px-2 py-1 text-white/40 transition hover:bg-white/10 hover:text-white disabled:opacity-40"
            aria-label="Kapat"
          >
            ✕
          </button>
        ...
        <div className="max-h-96 overflow-y-auto px-5 pb-5">
          {searching ? (
            <p className="py-8 text-center text-sm text-white/40">Aranıyor…</p>
          ) : null}
          ...
          <div className="space-y-2">
            {results.map((r) => (
              <button
                ...
                disabled={adding}
                className="group flex w-full items-center gap-3 rounded-xl border border-transparent p-3 text-left transition hover:border-white/10 hover:bg-white/5 disabled:opacity-60"
              >
                ...
                <div className="shrink-0">
                  {addingId === r.id && adding ? (
                    <span className="text-xs text-white/40">Ekleniyor…</span>
                  ) : (
                    <span className="inline-flex items-center gap-1 text-xs text-white/40 transition group-hover:text-amber-300">
